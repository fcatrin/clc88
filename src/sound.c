...
unsigned char pokey_buffer_0[POKEY_BUFFER_SIZE];
unsigned char pokey_buffer_1[POKEY_BUFFER_SIZE];
...
void sound_register_write(uint16 addr, uint8 val) {
	...
	pokey_update_sound(reg, val, chip, 64 / POKEY_CHIPS);
}
...
void sound_process() {
	...
	for(unsigned i=0; i<POKEY_BUFFER_SIZE && pokey_write_index < SOUND_BUFFER_SIZE; i++) {
		sound_buffer[pokey_write_index++] = pokey_buffer_0[i] << 8;
		sound_buffer[pokey_write_index++] = pokey_buffer_1[i] << 8;
	}
